...
const currentTimeFormatted = `${String(currentLondonTime.getHours()).padStart(
  2,
  "0"
)}${String(currentLondonTime.getMinutes()).padStart(2, "0")}`;
...
jQuery(document).ready(function () {
  ...
  jQuery(".dp2_people div, .people-dropdown").on("click tap", function (e) {
    ...
    jQuery(".musicbox-time a.category").attr(
      "pid",
      sday === 5 || sday === 6 || sday === 7 || sday === 1
        ? "113101480"
        : "114340933"
    );
    jQuery(".karaoke-time a.category").attr(
      "pid",
    ...
    jQuery(".choose_times .owl-carousel")
      .data("needed", people)
      .attr("data-needed", people);
    jQuery(".musicbox-time .owl-carousel")
      .data("needed", dataNeeded)
      .attr("data-needed", dataNeeded);
    jQuery(".racingsims-time .owl-carousel")
      .data("needed", Math.ceil(people / 6))
      .attr("data-needed", Math.ceil(people / 6));
  });
  ...
  jQuery(".item-times").on("change", function (e) {
    ...
    jQuery(".timegroup").each(function (index) {
      var tmeIndex = jQuery(this)
        .find('.btn-time[time^="' + thisTime.slice(0, -3) + '"]')
        .parent()
        .index();
  ...
  jQuery("#available_btn").on("click", function (e) {
    ...
    if (
      ...
      jQuery(".box-time span").text(
        jQuery(".item-times option:selected").val()
      );
      ...
      if (sday === 1 || sday === 7) {
        jQuery(".activities[title='Bottomless Brunch']").removeClass(
          "disabled"
        );
      } else {
  ...
  jQuery(".activities").on("click", function (e) {
    ...
    jQuery(".activities[title='Music Quiz']").toggleClass(
      "disabled",
      isKaraoke
    );
    ...
    if (sday == 1 || sday == 2 || sday == 3 || sday == 4 || sday == 5) {
      if (title === "Music Quiz") {
        jQuery(
          ".musicbox-time .btn-time[time='23:30'], .musicbox-time .btn-time[time='24:00']"
        ).addClass("disabled");
      } else if (title === "Racing Sims") {
        jQuery(
          ".racingsims-time .btn-time[time='23:30'], .racingsims-time .btn-time[time='24:00'], .racingsims-time .btn-time[time='00:30']"
        ).addClass("disabled");
      } else if (title === "Karaoke") {
        jQuery(
          ".karaoke-time .btn-time[time='23:30'], .karaoke-time .btn-time[time='24:00']"
        ).addClass("disabled");
      } else if (title === "Table Bookings") {
        jQuery(".table-bookings-time .btn-time[time='23:30']").addClass(
          "disabled"
        );
      } else if (title === "Music Arena") {
        jQuery(
          ".music-arena-time .btn-time[time='23:30'], .music-arena-time .btn-time[time='24:00']"
        ).addClass("disabled");
      }
    }
  });
  ...
  jQuery("#add_activities,#upgrade_btn").on("click", function (e) {
    ...
    if (currentSelection.length > 1) {
      ...
      jQuery("#upselltitle")
        .find("h2")
        .html("Would you like to add anything else?");
  ...
  jQuery("#activity_btn").on("click", function (e) {
    ...
    //console.log(currentSelection);
    if (
      currentSelection.indexOf("Music Quiz") >= 0 ||
      currentSelection.includes("Music Quiz")
    ) {
    ...
    document
      .querySelectorAll(".owl-carousel .btn-time")
  ...
  function disableOverlapping(currItem) {
    var se_time = jQuery(".btn-time.selected")
      .map(function () {
        return {
          start: jQuery(this).data("stime"),
          end: jQuery(this).data("etime"),
        };
      })
      .get();
  ...
  jQuery("#booking_btn, #continue_btn").click(function (e) {
    ...
    .map(group =>
        jQuery(`${group} .selected`)
            .map(function () {
                let pid;
                let totalPeople = people; // Declare totalPeople
                ...
                if (group === ".timegroup.musicbox-time") {
                    ...
                    // Debug: Log the pid
                    console.log('MusicBox PID:', pid);
                    return pid ? `&selected_products[${pid}]=${totalPeople}` : ''; 
                    ...
                    // Debug: Log the pid
                    console.log('Karaoke PID:', pid);
                    return pid ? `&selected_products[${pid}]=${people}` : ''; // Check if pid exists
                    ...
                    // Check conditions for peak day 
                    if (sday === 5 || sday === 6 || sday === 7 || sday === 1) { // Peak days (Friday-Sunday)
                        pid = 113109993;
                        if (people < 6) people = 6; // Minimum 6 people for peak day
                        totalPeople = people;
                    } else { // Off peak day
                        pid = 114481089;
                        if (people < 4) people = 4; // Minimum 4 people for off-peak day
                        totalPeople = people;
                    }
                
                    // Debug: Log the pid
                    console.log('RacingSim PID:', pid);
                    
                    return `&selected_products[${pid}]=${totalPeople}`; // Return the PID and the number of people
                }
                else if (group === ".timegroup.bottomless-time" || group === ".timegroup.table-bookings-time") {
                    pid = jQuery(this).attr("pid");

                    // Debug: Log the pid
                    console.log('Bottomless/Table PID:', pid);

                    // Ensure pid is defined before returning
                    if (pid === undefined) {
                        console.error("Missing pid for bottomless or table bookings time");
                    }

                    return pid ? `&selected_products[${pid}]=${people}` : ''; // Ensure pid exists
                } else {
                    pid = jQuery(this).attr("pid");

                    // Debug: Log the pid
                    console.log('Other Group PID:', pid);
...
// Ensure categoryToZoneMap is correct
const categoryToZoneMap = {
    ...
    var burl =
      "https://bookings.thespiderbox.com/book/?date=" +
      dte +
      "&category_id=" +
      zid +
      "&start_time=" +
...
function disableTodayPastTime(timeBuffer) {
  ...
  document.querySelectorAll(".btn-time.category.item").forEach((element) => {
    ...
    const adjustedCurrentTime = `${String(
      currentLondonTime.getHours() + bufferHours
    ).padStart(2, "0")}${String(
      currentLondonTime.getMinutes() + bufferMinutes
    ).padStart(2, "0")}`;
...
function scrollToAvailableTime() {
  document
    .querySelectorAll(".timegroup .owl-carousel")
    .forEach(function (carousel) {
      // Find the first '.btn-time' without the 'disabled-availability' class
      const availableTimes = Array.from(
        carousel.querySelectorAll(".btn-time:not(.disabled-availability)")
      );
      ...
      if (availableTimes.length > 0) {
        ...
        jQuery(carousel).trigger("to.owl.carousel", [
